...
const Setting = ({ active }: IActivableProps) => {
  // match Hamburger: bars use activeColor, knobs (dots) use activeAccentColor
  const barFill = active ? activeColor : inactiveColor
  const dotFill = active ? activeAccentColor : inactiveColor
  return (
    <svg
