...
export const bindPopupWithSceneCard = (
  layer: Layer,
  stringRenderer: typeof renderToString,
  { dgw, title }: BindPopupWithSceneCardOptions,
) => {
  // @ts-ignore
  ...
  layer.on('click', async () => {
    const sceneData = await dgw.getSceneDetailArticleByTitle(
      title,
      get(layer, 'feature.properties.url') as string | undefined,
    )
    const props = {
    ...
    // @ts-ignore
    bindPopupWithComponent(layer, stringRenderer, {
      Component: SceneCard,
      props,
    })
  })
