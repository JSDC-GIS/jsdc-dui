...
class ArticleProxyParser extends AbsctractArticleProxyParser implements IArticleProxyParser {
  ...
  async getAll (refresh?: boolean | undefined): Promise<SummaryArticleType[]> {
    ...
    cache.summaryMap = articles.reduce((obj, article) => {
  ...
  async getDetailByTitle (title: string, fallbackUrl?: string | null | undefined) {
  ...
  getArticlesFromHTML (dom: Document) {
    ...
    const result: SummaryArticleType[] = articleCards.map(cardElem => {
      ...
      const content: string = cardElem.querySelector('.sppb-article-introtext')?.innerHTML || 'null'
      const imgSrc: string = (cardElem.querySelector('a img') as HTMLImageElement).src || 'null'
  ...
  getExternalDetailFromHTML (dom: Document) {
    const newContent = (dom.querySelector('.sppb-addon-content p.d-h3') as HTMLHeadingElement)?.innerText
    const subtitle =  (dom.querySelector('.entry-header .tags') as HTMLDivElement)?.innerText
    const ref =  (dom.querySelector('.sppb-addon-content p:nth-child(4) span') as HTMLSpanElement)?.innerText
