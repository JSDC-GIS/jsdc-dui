...
export default class LayerController extends AbstractController {
    ...
    showByNames(names: Array<string>, hideOthers = false) {
        const ids = names.map(name => get(this.getByName(name), "id") ).filter( id => id !== undefined ) as Array<string>
        if(hideOthers) {
    ...
    setLayerList = () => {
        ...
        this.layerInfos = layers.map( layer => pick(layer, ['id', 'description', 'show']))
    ...
    getByName<P extends Layer> (name: string) {
        return this.layers.values.find( layer => layer.description.name === name) as JSDCLayer<P> | undefined
    }
    remove(layer: JSDCLayer, { removeFromMap = true }: RemoveOptions = {}) {
        removeFromMap && this.validateViewer().removeLayer(layer.isInstanceExistStrict().instance)
